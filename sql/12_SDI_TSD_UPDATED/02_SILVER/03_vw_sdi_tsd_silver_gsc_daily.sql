...
KEY MODELING NOTES:
  - Bronze preserves raw source text exactly as received
  - Standardization is applied here for filtering and regex classification
  ...
  - Final output dimensions are capitalized for downstream Gold consistency
...
WITH standardized AS (
    SELECT
        ...
        TRIM(query) AS query_trimmed,
        LOWER(TRIM(query)) AS query_classification,
...
classified AS (
    SELECT
        ...
        impressions,
        CASE
            WHEN query_trimmed IS NULL OR query_trimmed = '' THEN 'EXCLUDE'
