...
PURPOSE:
  Daily unattended QA for sdi_profound_bronze_citations_domain_daily.
...
BEGIN
  ...
  DECLARE tbl STRING DEFAULT 'sdi_profound_bronze_citations_domain_daily';
  ...
  dup_groups AS (
    ...
    FROM (
      SELECT account_id, root_domain, date_yyyymmdd, COUNT(*) c
      FROM bronze_scope GROUP BY 1,2,3 HAVING c > 1
    )
  )
  SELECT run_ts, test_dt, tbl, 'critical', 'duplicate_grain_groups_validation_weeks', 'HIGH',
         ...
         IF(dup_group_count=0,'No duplicate grain groups detected in Bronze.',
            'Duplicate grain groups detected for (account_id, root_domain, date_yyyymmdd).'),
  ...
  SELECT run_ts, test_dt, tbl, 'critical', 'null_key_or_date_rows_validation_weeks', 'HIGH',
         ...
         IF(bad_rows=0,'No NULL key/date rows found in Bronze for the validation weeks.',
            'Found NULL values in (account_id, root_domain, date_yyyymmdd, date).'),
  ...
  raw_clean AS (
    ...
    WHERE account_id IS NOT NULL AND root_domain IS NOT NULL AND date_yyyymmdd IS NOT NULL AND date IS NOT NULL
  ),
  raw_dedup AS (
  ...
  raw_keys AS (SELECT account_id, root_domain, date_yyyymmdd FROM raw_dedup),
  bronze_keys AS (
