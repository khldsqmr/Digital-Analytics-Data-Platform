-- ============================================================
-- SILVER 2: GRANULAR — BigQuery
-- Uses daily disaggregation via GENERATE_DATE_ARRAY
-- ============================================================
CREATE OR REPLACE VIEW
  `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_silver_spendGranular_weekly`
AS

WITH

actual_clean AS (
  SELECT Quarter, Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, FileLoad_Date,
  ...
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendActualsGranular_weekly`
...
forecast_clean AS (
  SELECT Quarter, Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, FileLoad_Date,
    ...
    weekly_forecast,
    UPPER(TRIM(week_type))     AS source_week_type
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendForecastsGranular_weekly`
  WHERE Week_Beginning_Monday IS NOT NULL AND Week_Ending_Sunday IS NOT NULL
),
...
valid_combinations AS (
  SELECT DISTINCT LOB_Supported, Channel, Tactic, Message_Type, Agency FROM aggregated
),
...
spine AS (
  SELECT
    ...
    cal.boundary_stub_date, cal.quarter_end_date,
...
with_spend_for_wow AS (
  ...
  LEFT JOIN metric_lookup stub_lkp
    ...
    AND stub_lkp.LOB_Supported = d.LOB_Supported AND stub_lkp.Channel = d.Channel
    AND stub_lkp.Tactic = d.Tactic AND stub_lkp.Message_Type = d.Message_Type AND stub_lkp.Agency = d.Agency
  LEFT JOIN metric_lookup prior_lkp
    ON  prior_lkp.qgp_date = d.wow_prior_qgp_date
    AND prior_lkp.LOB_Supported = d.LOB_Supported AND prior_lkp.Channel = d.Channel
    AND prior_lkp.Tactic = d.Tactic AND prior_lkp.Message_Type = d.Message_Type AND prior_lkp.Agency = d.Agency
  ...
  LEFT JOIN metric_lookup prior_stub_lkp
    ...
    AND prior_stub_lkp.LOB_Supported = d.LOB_Supported AND prior_stub_lkp.Channel = d.Channel
    AND prior_stub_lkp.Tactic = d.Tactic AND prior_stub_lkp.Message_Type = d.Message_Type AND prior_stub_lkp.Agency = d.Agency
)
