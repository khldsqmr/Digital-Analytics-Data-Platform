...
BEGIN
  ...
  -- Define which metrics must exist in long (edit to match your long build list)
  DECLARE metric_allowlist ARRAY<STRING> DEFAULT [
    'impressions','clicks','cost','all_conversions',
    'cart_start','postpaid_pspv'
  ];
  ...
  SELECT
    ...
    IF(duplicate_groups = 0, 'No duplicate long-daily grain detected.', 'Duplicate keys found in Gold long daily.'),
    IF(duplicate_groups = 0, 'No action required.', 'Fix long table build/unpivot; ensure uniqueness.'),
  ...
  WITH bad AS (
    ...
    WHERE date >= DATE_SUB(CURRENT_DATE(), INTERVAL lookback_days DAY)
      AND (
        account_id IS NULL OR campaign_id IS NULL OR date IS NULL OR metric_name IS NULL
      )
  )
  SELECT
    CURRENT_TIMESTAMP(), CURRENT_DATE(),
    ...
    IF(bad_rows = 0, '🟢', '🔴'),
    IF(bad_rows = 0, 'All long-daily identifiers are valid.', 'Null identifier(s) found in Gold long daily.'),
  ...
  SELECT
    ...
    'sdi_gold_sa360_campaign_daily_long',
    'critical',
    'Metric Name Allowlist Check (long daily)',
    'MEDIUM',
    0.0,
    CAST(bad_rows AS FLOAT64),
    CAST(bad_rows AS FLOAT64),
    IF(bad_rows = 0, 'PASS', 'FAIL'),
    IF(bad_rows = 0, '🟢', '🔴'),
    IF(bad_rows = 0, 'All metric_name values are expected.', 'Unexpected metric_name values found.'),
    IF(bad_rows = 0, 'No action required.', 'Update allowlist or fix long build metric naming.'),
    FALSE,
    IF(bad_rows = 0, TRUE, FALSE),
    IF(bad_rows > 0, TRUE, FALSE)
  FROM bad;
