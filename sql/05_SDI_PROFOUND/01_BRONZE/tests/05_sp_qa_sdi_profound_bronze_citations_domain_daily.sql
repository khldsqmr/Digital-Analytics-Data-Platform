...
===============================================================================*/
CREATE OR REPLACE PROCEDURE `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sp_qa_sdi_profound_bronze_citations_domain_daily`()
...
BEGIN
  ...
  WITH
  bronze_scope AS (
    SELECT * FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_citations_domain_daily`
  ...
  raw_dedup AS (
    SELECT * EXCEPT(rn) FROM (
      SELECT r.*,
        ROW_NUMBER() OVER (PARTITION BY account_id, root_domain, date_yyyymmdd
                           ORDER BY file_load_datetime DESC, filename DESC, insert_date DESC) rn
      FROM raw_clean r
    ) WHERE rn=1
  ),
  agg_raw AS (
    SELECT COUNT(1) row_cnt, SUM(cnt) sum_cnt, SUM(share_of_voice) sum_sov FROM raw_dedup
  ),
  agg_bronze AS (
    SELECT COUNT(1) row_cnt, SUM(count) sum_cnt, SUM(share_of_voice) sum_sov FROM bronze_scope
  ),
  crit_duplicates AS (
    SELECT COUNT(1) dup_groups FROM (
      SELECT account_id, root_domain, date_yyyymmdd, COUNT(*) c FROM bronze_scope GROUP BY 1,2,3 HAVING c>1
    )
  ),
  crit_nulls AS (
    SELECT COUNT(1) bad_rows
    FROM bronze_scope
    WHERE account_id IS NULL OR root_domain IS NULL OR date_yyyymmdd IS NULL OR date IS NULL
  ),
  crit_fresh AS (
  ...
  SELECT CURRENT_TIMESTAMP(), CURRENT_DATE(), 'sdi_profound_bronze_citations_domain_daily',
  ...
  FROM (
    ...
    SELECT 'reconciliation','raw_dedup_vs_bronze_metric_sums_iso_week','HIGH',
      ...
      CONCAT(
        'count exp=', CAST((SELECT IFNULL(sum_cnt,0) FROM agg_raw) AS STRING),
