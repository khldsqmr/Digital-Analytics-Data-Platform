-- ============================================================
-- SILVER 1: NON-GRANULAR / LOB-LEVEL — BigQuery
-- Uses daily disaggregation via GENERATE_DATE_ARRAY to
-- correctly split boundary weeks to BOUNDARY_STUB / BOUNDARY_FIRST
-- ============================================================
CREATE OR REPLACE VIEW
  `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_silver_spend_weekly`
AS

WITH

actual_clean AS (
  SELECT Quarter, Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week,
    FileLoad_Date, LOB_Supported, weekly_actual,
    UPPER(TRIM(week_type)) AS source_week_type
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendActuals_weekly`
  WHERE Week_Beginning_Monday IS NOT NULL AND Week_Ending_Sunday IS NOT NULL
),

forecast_clean AS (
  SELECT Quarter, Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week,
    FileLoad_Date, LOB_Supported, weekly_forecast,
    UPPER(TRIM(week_type)) AS source_week_type
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendForecasts_weekly`
  WHERE Week_Beginning_Monday IS NOT NULL AND Week_Ending_Sunday IS NOT NULL
),
...
source_joined AS (
  SELECT
    ...
    COALESCE(IF(a.FileLoad_Date > f.FileLoad_Date, a.FileLoad_Date, f.FileLoad_Date),
             a.FileLoad_Date, f.FileLoad_Date)                 AS FileLoad_Date,
    COALESCE(a.LOB_Supported, f.LOB_Supported)                 AS LOB_Supported,
    a.weekly_actual, f.weekly_forecast,
    COALESCE(IF(a.weekly_actual != 0, a.weekly_actual, NULL), f.weekly_forecast) AS weekly_display,
    UPPER(TRIM(COALESCE(a.source_week_type, f.source_week_type))) AS source_week_type
  FROM actual_clean a
  FULL OUTER JOIN forecast_clean f
    ON a.Quarter = f.Quarter AND a.QGP_Week = f.QGP_Week AND a.LOB_Supported = f.LOB_Supported
),

-- Derive quarter bounds from Source_Quarter string
...
with_bounds AS (
  SELECT *,
    DATE(
      CAST(CONCAT(CASE WHEN qtr_year_2digit < 50 THEN '20' ELSE '19' END,
        LPAD(CAST(qtr_year_2digit AS STRING), 2, '0'), '-',
        LPAD(CAST(((qtr_num - 1) * 3) + 1 AS STRING), 2, '0'), '-01') AS DATE)
    ) AS Source_Quarter_Start_Date,
    DATE_SUB(DATE_ADD(
      DATE(CAST(CONCAT(CASE WHEN qtr_year_2digit < 50 THEN '20' ELSE '19' END,
        LPAD(CAST(qtr_year_2digit AS STRING), 2, '0'), '-',
        LPAD(CAST(qtr_num * 3 AS STRING), 2, '0'), '-01') AS DATE)),
...
source_periods AS (
  SELECT *,
    IF(Week_Beginning_Monday > Source_Quarter_Start_Date, Week_Beginning_Monday, Source_Quarter_Start_Date) AS Period_Start,
    IF(Week_Ending_Sunday < Source_Quarter_End_Date, Week_Ending_Sunday, Source_Quarter_End_Date)           AS Period_End,
...
normalized_weekly AS (
  SELECT
    ...
    CASE WHEN source_week_type != 'BOUNDARY_WEEK' AND Week_Ending_Sunday > Source_Quarter_End_Date
      THEN ROUND(weekly_actual   * (DATE_DIFF(Source_Quarter_End_Date, Week_Beginning_Monday, DAY) + 1) / source_total_week_days, 2)
      ELSE weekly_actual   END AS weekly_actual,
    CASE WHEN source_week_type != 'BOUNDARY_WEEK' AND Week_Ending_Sunday > Source_Quarter_End_Date
      THEN ROUND(weekly_forecast * (DATE_DIFF(Source_Quarter_End_Date, Week_Beginning_Monday, DAY) + 1) / source_total_week_days, 2)
      ELSE weekly_forecast END AS weekly_forecast,
    CASE WHEN source_week_type != 'BOUNDARY_WEEK' AND Week_Ending_Sunday > Source_Quarter_End_Date
      THEN ROUND(weekly_display  * (DATE_DIFF(Source_Quarter_End_Date, Week_Beginning_Monday, DAY) + 1) / source_total_week_days, 2)
...
daily_with_qgp AS (
  SELECT
    ...
    LEAST(
      CASE
        WHEN DATE_ADD(calendar_date,
               INTERVAL MOD(7 - EXTRACT(DAYOFWEEK FROM calendar_date) + 7, 7) DAY)
             > DATE_SUB(DATE_ADD(DATE_TRUNC(calendar_date, QUARTER), INTERVAL 3 MONTH), INTERVAL 1 DAY)
        THEN DATE_SUB(DATE_ADD(DATE_TRUNC(calendar_date, QUARTER), INTERVAL 3 MONTH), INTERVAL 1 DAY)
        ELSE DATE_ADD(calendar_date,
               INTERVAL MOD(7 - EXTRACT(DAYOFWEEK FROM calendar_date) + 7, 7) DAY)
      END,
      Source_QGP_Week
    ) AS QGP_Week
...
-- Aggregate back to QGP_Week x LOB
aggregated AS (
  SELECT
...
-- Full spine: every QGP date x every LOB
spine AS (
  SELECT
...
dense_spend AS (
  SELECT
    ...
    s.boundary_stub_date, s.quarter_end_date, s.LOB_Supported,
    COALESCE(a.FileLoad_Date, MAX(a.FileLoad_Date) OVER (PARTITION BY s.LOB_Supported)) AS FileLoad_Date,
  ...
  FROM spine s
  LEFT JOIN aggregated a ON a.QGP_Week = s.qgp_date AND a.LOB_Supported = s.LOB_Supported
),
...
with_spend_for_wow AS (
  SELECT
    ...
    CASE
      ...
      WHEN d.week_type = 'BOUNDARY_FIRST' THEN COALESCE(d.spend_actual, 0) + COALESCE(stub_lkp.spend_actual, 0)
  ...
  LEFT JOIN metric_lookup prior_stub_lkp
    ON prior_stub_lkp.qgp_date = prior_cal.boundary_stub_date AND prior_stub_lkp.LOB_Supported = d.LOB_Supported
)
...
SELECT
  ...
  CASE WHEN spend_actual_for_wow IS NOT NULL AND spend_for_wow IS NOT NULL AND spend_for_wow != 0
    THEN ROUND(((spend_actual_for_wow - spend_for_wow) / spend_for_wow) * 100, 2)
    ELSE NULL END AS spend_actual_wow_pct,
  CASE WHEN week_type IN ('BOUNDARY_STUB','BOUNDARY_FIRST') THEN 'BOUNDARY_WEEK' ELSE 'NORMAL' END AS week_type,
