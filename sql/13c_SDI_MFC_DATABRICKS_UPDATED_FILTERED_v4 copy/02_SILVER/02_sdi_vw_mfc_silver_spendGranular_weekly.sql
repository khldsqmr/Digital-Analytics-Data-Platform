...
BEGIN
  ...
  actual_clean AS (
    SELECT
      Quarter, QGP_Week, FileLoad_Date,
  ...
  -- One row per QGP_Week x combination — prefer actuals, then latest FileLoad_Date
  source_joined AS (
    SELECT * FROM (
      SELECT
  ...
  -- All combinations that ever appeared in source
  ...
  -- Full spine: every QGP date (incl BOUNDARY_STUB) x every combination
  spine AS (
    SELECT
    ...
    WHERE cal.qgp_date >= (SELECT MIN(QGP_Week) FROM source_joined WHERE QGP_Week IS NOT NULL)
      AND cal.qgp_date <= (SELECT MAX(QGP_Week) FROM source_joined WHERE QGP_Week IS NOT NULL)
  ),

  -- Aggregate source to one spend per QGP_Week x combination
  aggregated AS (
    SELECT
      QGP_Week,
  ...
  dense_spend AS (
    SELECT
      ...
      -- Actual gated by is_complete_period; forecast always shown
  ...
  -- WoW computation using PulseTMS stub lookup logic
  with_spend_for_wow AS (
    ...
    -- Stub value for BOUNDARY_FIRST numerator (e.g. Mar 31 value for Apr 4)
    ...
    -- Prior week value for denominator (e.g. Mar 28 for Apr 4)
    ...
    -- Prior week's calendar row to get its boundary_stub_date (for 2nd week denominator)
    LEFT JOIN prdrzranalytics.lab42.sdi_vw_mfc_dim_qgp_calendar prior_cal
      ON prior_cal.qgp_date = d.wow_prior_qgp_date

    -- Stub value for prior week (e.g. Mar 31 for Apr 11 denominator)
