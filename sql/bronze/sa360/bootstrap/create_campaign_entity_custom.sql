...
PURPOSE:
  Create a clean Bronze campaign entity/settings snapshot table:
    - Canonical DATE parsed from date_yyyymmdd
    - Standardized naming (campaign_name instead of generic name)
    - Partition + cluster for efficient joins to Daily fact
...
CLUSTER BY account_id, campaign_id
OPTIONS(
  description = "Bronze SA360 campaign entity/settings snapshot. Cleaned naming, strong schema descriptions, partitioned by canonical date."
);
