...
ALIGNMENT (matches merge SP exactly):
  - RAW source: prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_citations_topic_tag_daily_tmo
  - RAW canonical date: SAFE.PARSE_DATE('%Y%m%d', raw.date_yyyymmdd)
  - RAW dedup grain: (account_id, root_domain, topic, tag, date_yyyymmdd)
...
BEGIN
  ...
  DECLARE bronze_table STRING DEFAULT 'sdi_profound_bronze_citations_topic_tag_daily';
  ...
  WITH dup AS (
    ...
    FROM (
      SELECT account_id, root_domain, topic, tag, date_yyyymmdd, COUNT(*) AS c
