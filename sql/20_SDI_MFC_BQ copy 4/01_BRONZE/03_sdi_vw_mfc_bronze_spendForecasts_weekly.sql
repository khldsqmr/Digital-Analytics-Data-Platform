...
BEGIN
  ...
  OPTIONS (
    description = 'MFC Bronze Forecast (non-granular / LOB level), ranked at campaign grain then rolled up. Includes legacy LOB codes HSI (Broadband) and TBG (TFB), normalized to canonical values.'
  )
  AS
  WITH raw AS (
    SELECT
