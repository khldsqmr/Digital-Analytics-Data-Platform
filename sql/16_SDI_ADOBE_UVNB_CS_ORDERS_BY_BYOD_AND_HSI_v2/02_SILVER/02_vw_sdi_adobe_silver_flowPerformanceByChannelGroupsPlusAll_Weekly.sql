...
BUSINESS RULES:
  ...
  - ChannelGroup rows use Bronze LTC Groups flow metrics and Bronze LTC Groups total UVNB.
  - UvnbTotalAdobe comes from the total UVNB stream (Bronze uvnbTotalByAll / uvnbTotalByLtcGroups).
  ...
  - EcCompleted / EcSuccessful are Adobe Experience Cloud metrics, passed through
    directly after the UVNB and Cartstart metrics (before Orders) — no derivation, independent
    of the UVNB/Cartstart/Orders funnel.
...
DOWNSTREAM:
  Gold: vw_sdi_adobe_gold_flowPerformanceByChannelGroups_weekly
================================================================================================= */
...
WITH FlowRows AS (
  ...
  TotalUvnbRows AS (
    ...
    SELECT
      ...
      LtcGroup AS ChannelGroup,
      UvnbTotalAdobe
    FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.vw_sdi_adobe_bronze_uvnbTotalByLtcGroups_Weekly`
  )
