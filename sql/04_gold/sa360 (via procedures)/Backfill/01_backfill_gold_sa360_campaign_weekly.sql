...
NOTES:
  ...
  - Excludes future qgp_week buckets (qgp_week > CURRENT_DATE()).
  - Preserves metadata: file_load_datetime (MAX of daily rows in bucket).
===============================================================================
*/
...
USING (
  ...
  -- Defensive dedupe in case daily has multiple rows for same acct/campaign/date
  daily_dedup AS (
    SELECT * EXCEPT(rn)
    FROM (
      SELECT
  ...
  impacted_qgp_weeks AS (
    -- Only rebuild buckets impacted by the requested backfill date range
    -- and never build future qgp_week buckets
    SELECT DISTINCT qgp_week
    FROM bucketed
    WHERE date BETWEEN backfill_start_date AND backfill_end_date
      AND qgp_week <= CURRENT_DATE()
  ),
  ...
  agg AS (
    SELECT
      ...
      -- Pick latest non-null dimension value in bucket (deterministic tie-breaker)
      ARRAY_AGG(lob IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS lob,
      ARRAY_AGG(ad_platform IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS ad_platform,
      ARRAY_AGG(account_name IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS account_name,
      ARRAY_AGG(campaign_name IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS campaign_name,
      ARRAY_AGG(campaign_type IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS campaign_type,
      ARRAY_AGG(advertising_channel_type IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS advertising_channel_type,
      ARRAY_AGG(advertising_channel_sub_type IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS advertising_channel_sub_type,
      ARRAY_AGG(bidding_strategy_type IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS bidding_strategy_type,
      ARRAY_AGG(serving_status IGNORE NULLS ORDER BY date DESC, file_load_datetime DESC LIMIT 1)[SAFE_OFFSET(0)] AS serving_status,
