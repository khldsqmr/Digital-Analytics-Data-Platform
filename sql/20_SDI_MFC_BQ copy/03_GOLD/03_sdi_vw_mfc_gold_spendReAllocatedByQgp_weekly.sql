CREATE OR REPLACE VIEW `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_gold_spendReAllocatedByQgp_weekly` AS
...
silver_latest AS (
  SELECT s.*
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_silver_spend_weekly` s
  JOIN latest_file f
...
aggregated AS (
  ...
  FROM daily_with_qgp
  GROUP BY QGP_Date, LOB_Supported
)
...
SELECT
  ...
  spend_display
FROM aggregated
ORDER BY QGP_Date DESC, LOB_Supported;