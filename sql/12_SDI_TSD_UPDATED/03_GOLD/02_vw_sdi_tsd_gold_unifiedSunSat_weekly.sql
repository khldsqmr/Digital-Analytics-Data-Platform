...
PURPOSE:
  Unified Gold weekly reporting mart for the Total Search Dashboard
  using Sunday-to-Saturday weeks.
...
KEY MODELING NOTES:
  - NULL-aware aggregation throughout
  - mais spend columns added alongside existing metrics
  - mais_platform_spend: NULL for PAID SEARCH, populated for other channels
  - mais sub-channels: populated for PAID SEARCH only
...
SELECT
    ...
    -- NEW: mais spend
    CASE WHEN COUNT(mais_platform_spend) = 0            THEN NULL ELSE SUM(mais_platform_spend) END             AS mais_platform_spend,
    CASE WHEN COUNT(mais_platform_spend_branded) = 0    THEN NULL ELSE SUM(mais_platform_spend_branded) END     AS mais_platform_spend_branded,
    CASE WHEN COUNT(mais_platform_spend_nonbranded) = 0 THEN NULL ELSE SUM(mais_platform_spend_nonbranded) END  AS mais_platform_spend_nonbranded,
    CASE WHEN COUNT(mais_platform_spend_pla) = 0        THEN NULL ELSE SUM(mais_platform_spend_pla) END         AS mais_platform_spend_pla,
    CASE WHEN COUNT(mais_platform_spend_pmax) = 0       THEN NULL ELSE SUM(mais_platform_spend_pmax) END        AS mais_platform_spend_pmax

FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.vw_sdi_tsd_gold_unified_daily`
GROUP BY weekSunToSat, lob, channel;