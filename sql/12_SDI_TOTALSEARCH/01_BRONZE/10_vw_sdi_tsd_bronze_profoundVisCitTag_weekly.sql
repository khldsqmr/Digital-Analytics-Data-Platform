/* =================================================================================================
FILE: 10_vw_sdi_tsd_bronze_profoundVisCitTag_weekly.sql
LAYER: Bronze View
DATASET: prj-dbi-prd-1.ds_dbi_digitalmedia_automation
VIEW: vw_sdi_tsd_bronze_profoundVisCitTag_weekly

SOURCES:
  prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_vis_tag_weekly_sunday
  prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_cit_tag_weekly_sunday
  prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_gofish_vis_tag_weekly_sunday
  prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_gofish_cit_tag_weekly_sunday

DESTINATION:
  prj-dbi-prd-1.ds_dbi_digitalmedia_automation.vw_sdi_tsd_bronze_profoundVisCitTag_weekly

PURPOSE:
  Canonical Bronze weekly ProFound / GoFish source mart for the Total Search Dashboard.
  This view standardizes weekly visibility and citation tag-level data into one source-close object.
...
CREATE OR REPLACE VIEW `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.vw_sdi_tsd_bronze_profoundVisCitTag_weekly`
AS
...
WITH vis_nonbrand_base AS (
    ...
    FROM `prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_vis_tag_weekly_sunday`
...
cit_nonbrand_base AS (
    SELECT
        ...
        TIMESTAMP(File_Load_datetime) AS file_load_datetime,
        Filename AS filename
    FROM `prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_cit_tag_weekly_sunday`
    WHERE UPPER(TRIM(tag)) = 'LOB - POSTPAID'
...
vis_brand_base AS (
    SELECT
        ...
        TIMESTAMP(File_Load_datetime) AS file_load_datetime,
        Filename AS filename
    FROM `prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_gofish_vis_tag_weekly_sunday`
    WHERE UPPER(TRIM(tag)) = 'LOB - POSTPAID'
      AND UPPER(TRIM(asset_name)) IN ('T-MOBILE', 'AT&T', 'VERIZON')
),
...
cit_brand_base AS (
    SELECT
        ...
        TIMESTAMP(File_Load_datetime) AS file_load_datetime,
        Filename AS filename
    FROM `prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_gofish_cit_tag_weekly_sunday`
    WHERE UPPER(TRIM(tag)) = 'LOB - POSTPAID'
      AND LOWER(TRIM(root_domain)) IN ('t-mobile.com', 'att.com', 'verizon.com')
),
