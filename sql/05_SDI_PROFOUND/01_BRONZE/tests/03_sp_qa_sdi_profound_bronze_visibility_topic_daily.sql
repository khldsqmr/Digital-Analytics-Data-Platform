...
BEGIN
  ...
  WITH x AS (SELECT MAX(file_load_datetime) max_fldt FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_visibility_topic_daily`),
  ...
  -- Null key/date
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH z AS (SELECT COUNTIF(account_id IS NULL OR asset_name IS NULL OR topic IS NULL OR date_yyyymmdd IS NULL OR date IS NULL) bad_rows FROM _bronze_window)
  ...
  -- Duplicate grain (account_id + asset_name + topic + date_yyyymmdd)
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH d AS (SELECT COUNT(*) dup_groups FROM (SELECT account_id, asset_name, topic, date_yyyymmdd FROM _bronze_window GROUP BY 1,2,3,4 HAVING COUNT(*)>1))
  ...
  -- Recon metric sums (visibility checksum)
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH r AS (SELECT SUM(executions) exec_sum,SUM(mentions_count) mentions_sum,SUM(share_of_voice) sov_sum,SUM(visibility_score) vis_sum FROM _raw_dedup),
       ...
       t AS (
         SELECT (COALESCE(r.exec_sum,0)+COALESCE(r.mentions_sum,0)+COALESCE(r.sov_sum,0)+COALESCE(r.vis_sum,0)) expected_value,
