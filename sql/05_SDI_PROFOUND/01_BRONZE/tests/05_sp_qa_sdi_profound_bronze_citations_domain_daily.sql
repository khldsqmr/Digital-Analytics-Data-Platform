...
BEGIN
  ...
  WITH x AS (SELECT MAX(file_load_datetime) max_fldt FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_citations_domain_daily`),
  ...
  -- Null key/date
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH z AS (SELECT COUNTIF(account_id IS NULL OR root_domain IS NULL OR date_yyyymmdd IS NULL OR date IS NULL) bad_rows FROM _bronze_window)
  ...
  -- Duplicate grain (account_id + root_domain + date_yyyymmdd)
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH d AS (SELECT COUNT(*) dup_groups FROM (SELECT account_id, root_domain, date_yyyymmdd FROM _bronze_window GROUP BY 1,2,3 HAVING COUNT(*)>1))
  ...
  -- Recon metric sums (citations checksum = count + share_of_voice)
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
