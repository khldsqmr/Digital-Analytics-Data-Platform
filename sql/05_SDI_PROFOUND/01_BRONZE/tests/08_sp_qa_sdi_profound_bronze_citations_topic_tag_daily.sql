...
===============================================================================*/
CREATE OR REPLACE PROCEDURE `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sp_qa_sdi_profound_bronze_citations_topic_tag_daily`()
...
BEGIN
  ...
  raw_dedup AS (
    SELECT * EXCEPT(rn) FROM (
      SELECT r.*,
        ROW_NUMBER() OVER (PARTITION BY account_id, root_domain, topic, tag, date_yyyymmdd
  ...
  crit_duplicates AS (
    SELECT COUNT(1) dup_groups FROM (
      SELECT account_id, root_domain, topic, tag, date_yyyymmdd, COUNT(*) c FROM bronze_scope GROUP BY 1,2,3,4,5 HAVING c>1
    )
  ),
  crit_nulls AS (
  ...
  SELECT CURRENT_TIMESTAMP(), CURRENT_DATE(), 'sdi_profound_bronze_citations_topic_tag_daily',
