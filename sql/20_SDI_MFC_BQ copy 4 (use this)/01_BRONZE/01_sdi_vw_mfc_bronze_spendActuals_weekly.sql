...
BEGIN
  ...
  OPTIONS (
    description = 'MFC Bronze Actuals (non-granular / LOB level). For each (Quarter, QGP_Week, LOB), uses only the single most recent file that has any actual data. Includes legacy LOB codes HSI (Broadband) and TBG (TFB), normalized to canonical values.'
  )
  AS
  WITH raw AS (
    SELECT
