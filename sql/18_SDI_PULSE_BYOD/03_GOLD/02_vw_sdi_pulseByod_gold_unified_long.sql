...
KEY MODELING NOTES:
  ...
  - data_source and channel flow from Gold Wide per-source metadata
    columns — future-proof for Adobe multi-channel sources
  - max_data_date: MAX(source_max_data_date) OVER (PARTITION BY data_source)
...
WITH unpivoted AS (
    ...
    SELECT week_sun_to_sat,
        profound_data_source    AS data_source,
        profound_channel        AS channel,
