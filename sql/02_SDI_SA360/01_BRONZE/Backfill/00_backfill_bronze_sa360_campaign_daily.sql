/*
===============================================================================
FILE: 00_backfill_bronze_sa360_campaign_daily.sql
LAYER: Bronze (One-time Backfill)
TARGET: prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_bronze_sa360_campaign_daily
SOURCE: prj-dbi-prd-1.ds_dbi_improvado_master.google_search_ads_360_campaigns_tmo

PURPOSE:
  Backfill historical Bronze Daily using the EXACT SAME logic as your incremental
  procedure, but over a configurable date range and in safe chunks.

NOTES:
  - This keeps your canonical date parsing + dedupe by (file_load_datetime, filename).
  - Processes in chunks (default 14 days) to reduce cost/risk.
===============================================================================
*/
...
DECLARE chunk_days          INT64 DEFAULT 14;                  -- safe chunk size
...
LOOP
  ...
  -- --------------------------------------------------------------------------
  -- BACKFILL MERGE: Bronze Daily (same logic as sp_merge_bronze_sa360_campaign_daily)
  -- --------------------------------------------------------------------------
