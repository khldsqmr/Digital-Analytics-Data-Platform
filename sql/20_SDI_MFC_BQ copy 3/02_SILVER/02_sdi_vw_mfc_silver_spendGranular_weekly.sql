...
BEGIN
  ...
  daily_spend AS (
    SELECT
      ...
      weekly_actual
        / (DATE_DIFF(Period_End, Period_Start, DAY) + 1) AS daily_actual,

      weekly_forecast
        / (DATE_DIFF(Period_End, Period_Start, DAY) + 1) AS daily_forecast,

      weekly_display
        / (DATE_DIFF(Period_End, Period_Start, DAY) + 1) AS daily_display,
  ...
  with_spend_for_wow AS (
    ...
    -- Prior week's calendar row to identify its stub date
