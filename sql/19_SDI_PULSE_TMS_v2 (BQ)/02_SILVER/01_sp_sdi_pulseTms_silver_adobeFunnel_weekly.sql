...
BEGIN
  ...
  -- ---------------------------------------------------------------------------
  -- 2A: Metric lookups
  --     MetricLookup supports WoW boundary logic by qgp_date.
  --     LYWeeklyLookup supports LY/YoY by prior-year same ISO week total.
  -- ---------------------------------------------------------------------------
  MetricLookup AS (
  ...
  SELECT
    ...
    CASE
      ...
      ELSE SAFE_DIVIDE(u.metric_value, ly_week.ly_weekly_metric_value) - 1
    END                                                                   AS yoy_pct,
