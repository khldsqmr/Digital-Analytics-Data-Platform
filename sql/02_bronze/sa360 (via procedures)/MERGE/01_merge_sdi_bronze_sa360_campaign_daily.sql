...
LAYER: Bronze
PROC:  prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sp_merge_bronze_sa360_campaign_daily

PURPOSE:
  Incrementally upsert recent SA360 daily snapshots into Bronze:
    - lookback window for late-arriving files
    - dedup within window by (file_load_datetime desc, filename desc)
    - enforce canonical date from date_yyyymmdd
    - prevent partition garbage (drop unparseable dates)

MERGE KEY:
  (account_id, campaign_id, date_yyyymmdd)

DEDUPE (only place we dedupe):
  ROW_NUMBER() over MERGE KEY ordered by file_load_datetime DESC, filename DESC
