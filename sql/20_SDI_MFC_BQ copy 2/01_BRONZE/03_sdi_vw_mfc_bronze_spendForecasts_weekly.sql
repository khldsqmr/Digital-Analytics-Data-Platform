...
-- Converted from Databricks sdi_sp_mfc_bronze_spendForecasts_weekly
--
-- Logic:
--   1. Pull both Forecast and Actual Spend columns from raw
--   2. Aggregate weekly_forecast per (Quarter, QGP_Week, LOB, FileLoad_Date)
--   3. Keep only the latest forecast snapshot filed BEFORE the first
--      actual arrived for that key (pre-actual forecasts only)
--   4. For boundary weeks where only one quarter's forecast exists,
--      derive a pro-rated forecast for the missing quarter:
--      weekly_forecast / source_days * missing_days
-- ============================================================
...
BEGIN
  ...
  OPTIONS (
    description = 'MFC Bronze — refreshed via sdi_sp_mfc_bronze_spendForecasts_weekly.'
  )
  AS
  ...
  -- First date on which an actual arrived for each (Quarter, QGP_Week, LOB)
  ...
  -- Keep only forecasts filed before the first actual; rank by latest first
  ranked AS (
    SELECT
      s.*,
      ROW_NUMBER() OVER (
  ...
  -- QGP_Weeks that have at least one actual (used to detect boundary gaps)
  actuals_quarters AS (
  ...
  -- Flag boundary weeks (QGP_Week appears under two different Quarters)
  week_type AS (
    SELECT
      QGP_Week,
      CASE
  ...
  -- Identify which quarter-end date falls inside each boundary week
  boundary_dates AS (
  ...
  -- For boundary weeks with only one quarter's forecast, calculate
  -- source_days and missing_days for pro-ration
  boundary_with_forecast AS (
    ...
    FROM (
      ...
      -- Only process when the other quarter's forecast does NOT already exist in best
      WHERE NOT EXISTS (
        SELECT 1
        FROM best ob
  ...
  -- Pro-rate forecast for the missing quarter: (daily_rate × missing_days)
  ...
  FROM (
    ...
    -- Derived pro-rated forecasts for missing boundary-week quarters
    SELECT
      Quarter,
      Week_Beginning_Monday,
      Week_Ending_Sunday,
