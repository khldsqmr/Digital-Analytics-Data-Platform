...
COMMENT 'Creates/refreshes sdi_tbl_mfc_bronze_spendForecastGranular_weekly. Refreshed weekly. Uses latest-file-snapshot selection per (Quarter, QGP_Week, LOB) — no per-campaign fallback across files.'
BEGIN
  CREATE OR REPLACE TABLE
    prdrzranalytics.lab42.sdi_tbl_mfc_bronze_spendForecastGranular_weekly
  USING DELTA
  COMMENT 'MFC Bronze Forecast (granular). For each (Quarter, QGP_Week, LOB), uses only the single most recent file that has any forecast data. Includes legacy LOB codes HSI (Broadband) and TBG (TFB), normalized to canonical values.'
  AS
  WITH raw AS (
    SELECT
      Quarter,
  ...
  SELECT
    ...
    b.FileLoad_Date, b.Source_File_Date, b.LOB_Supported, b.Channel, b.Tactic, b.Message_Type, b.Agency,
    b.weekly_forecast, w.week_type
  FROM best b
