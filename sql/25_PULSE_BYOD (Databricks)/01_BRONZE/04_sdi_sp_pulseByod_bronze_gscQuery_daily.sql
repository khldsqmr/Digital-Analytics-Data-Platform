...
WITH ranked AS (
    SELECT
        ...
        TRY_CAST(raw.site_url AS STRING)    AS site_url,
        TRY_CAST(raw.page AS STRING)    AS page,
        ...
        TRY_CAST(raw.query AS STRING)    AS query,
        TRY_CAST(raw.search_type AS STRING)    AS search_type,
        ...
        TRY_CAST(raw.position AS DOUBLE)                         AS position,
        TRY_CAST(raw.sum_position AS DOUBLE)                         AS sum_position,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY
                ...
                TRY_CAST(raw.site_url AS STRING),
                TRY_CAST(raw.page AS STRING),
                TRY_CAST(raw.query AS STRING),
                TRY_CAST(raw.search_type AS STRING),
                CAST(raw.date_yyyymmdd    AS STRING)
            ORDER BY
                CAST(raw.File_Load_datetime AS TIMESTAMP)       DESC,
                raw.Filename                            DESC,
                TRY_CAST(raw.__insert_date AS BIGINT)  DESC,
                TRY_CAST(raw.impressions AS DOUBLE) DESC,
                TRY_CAST(raw.clicks AS DOUBLE) DESC
        ) AS rn

    FROM prd_dbi_analytics.improvado.google_search_console_query_search_type_tmo raw
