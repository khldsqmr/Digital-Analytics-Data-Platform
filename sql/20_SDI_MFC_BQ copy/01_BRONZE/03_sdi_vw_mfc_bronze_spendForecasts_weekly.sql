...
CREATE OR REPLACE VIEW
  `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendForecasts_weekly`
AS
...
weekly_snapshots AS (
  ...
  GROUP BY Quarter, Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, FileLoad_Date, LOB_Supported
),

first_actual_date AS (
  SELECT Quarter, QGP_Week, LOB_Supported,
...
boundary_with_forecast AS (
  ...
  FROM (
    SELECT
      ...
      b.FileLoad_Date, b.LOB_Supported, b.weekly_forecast AS source_forecast,
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM best ob WHERE ob.QGP_Week = b.QGP_Week AND ob.Quarter != b.Quarter AND ob.LOB_Supported = b.LOB_Supported
    )
  )
...
derived_forecasts AS (
  ...
  LEFT JOIN best existing ON existing.QGP_Week = bwf.QGP_Week AND existing.Quarter = aq.Quarter AND existing.LOB_Supported = bwf.LOB_Supported
  WHERE existing.Quarter IS NULL
)

SELECT * FROM (
  SELECT b.Quarter, b.Week_Beginning_Monday, b.Week_Ending_Sunday, b.QGP_Week,
    b.FileLoad_Date, b.LOB_Supported, b.weekly_forecast, w.week_type, FALSE AS is_derived
