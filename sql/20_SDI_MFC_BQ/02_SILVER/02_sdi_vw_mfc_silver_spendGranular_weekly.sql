CREATE OR REPLACE VIEW `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_silver_spendGranular_weekly` AS

WITH joined AS (
  SELECT
    a.Quarter, a.Week_Beginning_Monday, a.Week_Ending_Sunday, a.QGP_Week,
    a.FileLoad_Date, a.LOB_Supported, a.Channel, a.Tactic, a.Message_Type, a.Agency,
    a.weekly_actual, f.weekly_forecast,
    COALESCE(NULLIF(a.weekly_actual, 0), f.weekly_forecast) AS weekly_display,
    a.week_type
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendActualsGranular_weekly` a
  LEFT JOIN `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendForecastsGranular_weekly` f
    ON a.Quarter = f.Quarter AND a.QGP_Week = f.QGP_Week
   AND a.LOB_Supported = f.LOB_Supported AND a.Channel = f.Channel
   AND a.Tactic = f.Tactic AND a.Message_Type = f.Message_Type AND a.Agency = f.Agency
),

forecast_only AS (
  SELECT
    f.Quarter, f.Week_Beginning_Monday, f.Week_Ending_Sunday, f.QGP_Week,
    f.FileLoad_Date, f.LOB_Supported, f.Channel, f.Tactic, f.Message_Type, f.Agency,
    NULL AS weekly_actual, f.weekly_forecast, f.weekly_forecast AS weekly_display,
    f.week_type
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendForecastsGranular_weekly` f
  LEFT JOIN `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendActualsGranular_weekly` a
    ON f.Quarter = a.Quarter AND f.QGP_Week = a.QGP_Week
   AND f.LOB_Supported = a.LOB_Supported AND f.Channel = a.Channel
   AND f.Tactic = a.Tactic AND f.Message_Type = a.Message_Type AND f.Agency = a.Agency
