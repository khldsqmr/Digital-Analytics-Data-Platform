...
forecast_only AS (
  ...
  LEFT JOIN `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_vw_mfc_bronze_spendActualsGranular_weekly` a
   ...
   AND f.Tactic        = a.Tactic
   AND f.Message_Type  = a.Message_Type
   AND f.Agency        = a.Agency
...
FROM with_proration
WHERE weekly_display IS NOT NULL
  AND weekly_display != 0
