...
PURPOSE:
  Create a clean, standardized Bronze daily campaign performance table:
    - Canonical DATE is parsed from date_yyyymmdd
    - Strong column descriptions for downstream usability
    - Partition + cluster for cost/perf

KEY STANDARDIZATION RULES:
  1) Canonical date:
       date = SAFE.PARSE_DATE('%Y%m%d', date_yyyymmdd)
     NOTE: We intentionally do NOT keep the raw INT64 `date` field anymore.

  2) Column cleanup:
     - __insert_date -> insert_date
     - _ma_hint_ec__eligibility__check_ -> ma_hint_ec_eligibility_check
     - trailing underscores removed in Bronze names (e.g., cart__start_ -> cart_start)

  3) TMO naming enforcement:
     Any source naming variant like t_mobile / t__mobile / t-mobile is standardized to `tmo`
     in Bronze column names.
     Example:
       raw.t__mobile__prepaid__low__funnel__prospect -> tmo_prepaid_low_funnel_prospect

  4) TBG and TFB are treated as the same family:
     Standardize TBG metrics into TFB columns in Bronze:
...
(
  ...
  date DATE OPTIONS(description="Canonical DATE parsed from date_yyyymmdd. This is the partition key."),
