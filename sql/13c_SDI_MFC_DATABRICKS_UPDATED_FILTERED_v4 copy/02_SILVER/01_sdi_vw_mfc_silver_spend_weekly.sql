...
--   PulseTMS-style stub lookup WoW.
-- ============================================================
...
source_joined AS (
  SELECT
    ...
    UPPER(TRIM(COALESCE(a.week_type, f.week_type)))     AS source_week_type
...
-- ── Cross-join spine: every QGP date x every LOB ─────────────────────────────
lob_universe AS (
  SELECT DISTINCT LOB_Supported FROM source_joined
),
...
spine AS (
  ...
  WHERE cal.qgp_date >= DATE'2020-01-01'
),

-- ── Aggregate source to one spend value per QGP_Week x LOB ───────────────────
aggregated AS (
  SELECT
...
SELECT
  ...
  CASE WHEN week_type = 'BOUNDARY_STUB' THEN 'BOUNDARY_WEEK'
       WHEN week_type = 'BOUNDARY_FIRST' THEN 'BOUNDARY_WEEK'
...
WHERE qgp_date >= (SELECT MIN(Source_QGP_Week) FROM source_joined WHERE Source_QGP_Week IS NOT NULL)
;