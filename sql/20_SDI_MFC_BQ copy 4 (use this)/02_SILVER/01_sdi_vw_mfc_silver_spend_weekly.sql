...
BEGIN
  ...
  OPTIONS (
    description = 'MFC Silver Spend (non-granular / LOB level). Spend_Actual and Spend_Forecast only — no blended Spend_Final. Boundary weeks use raw quarter tags from the source: if one quarter has a value and the other does not, only the quarter with a value is populated; if both have values, each carries its own raw value; if neither has a value, both are NULL. No day-proportional split is applied.'
  )
  AS
  WITH
