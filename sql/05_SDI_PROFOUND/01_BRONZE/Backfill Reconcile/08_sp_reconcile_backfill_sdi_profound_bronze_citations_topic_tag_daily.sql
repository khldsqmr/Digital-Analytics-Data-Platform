/* =================================================================================================
FILE: 08_sp_reconcile_backfill_sdi_profound_bronze_citations_topic_tag_daily.sql
LAYER: Bronze
PROC:  sp_reconcile_backfill_sdi_profound_bronze_citations_topic_tag_daily
TARGET TABLE: sdi_profound_bronze_citations_topic_tag_daily

PURPOSE:
  Reconcile Bronze ProFound Citations Topic+Tag Daily against raw for a supplied business-date range.
...
`prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sp_reconcile_backfill_sdi_profound_bronze_citations_topic_tag_daily`(
  p_start_date DATE,
  p_end_date   DATE
)
...
BEGIN
  ...
  MERGE `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_citations_topic_tag_daily` AS T
  USING (
    WITH src AS (
      SELECT
      ...
      FROM `prj-dbi-prd-1.ds_dbi_improvado_master.sdi_seo_profound_citations_topic_tag_daily_tmo` AS raw
    ),
