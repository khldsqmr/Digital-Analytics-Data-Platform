/* =================================================================================================
FILE: 04_sp_reconcile_backfill_sdi_profound_bronze_visibility_topic_tag_daily.sql
LAYER: Bronze
PROC:  sp_reconcile_backfill_sdi_profound_bronze_visibility_topic_tag_daily
TARGET TABLE: sdi_profound_bronze_visibility_topic_tag_daily

PURPOSE:
  Reconcile Bronze ProFound Visibility Topic+Tag Daily against raw for a supplied business-date range.
...
`prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sp_reconcile_backfill_sdi_profound_bronze_visibility_topic_tag_daily`(
  p_start_date DATE,
  p_end_date   DATE
)
    ...
    dedup AS (
      ...
      FROM (
        SELECT
          c.*,
          ROW_NUMBER() OVER (
            PARTITION BY account_id, asset_id, topic, tag, date_yyyymmdd
