...
BEGIN
  ...
  WITH x AS (SELECT MAX(file_load_datetime) max_fldt FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_citations_topic_tag_daily`),
  ...
  -- Null key/date
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH z AS (SELECT COUNTIF(account_id IS NULL OR root_domain IS NULL OR topic IS NULL OR tag IS NULL OR date_yyyymmdd IS NULL OR date IS NULL) bad_rows FROM _bronze_window)
  ...
  -- Duplicate grain
  INSERT INTO `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_profound_bronze_test_results`
  WITH d AS (SELECT COUNT(*) dup_groups FROM (SELECT account_id, root_domain, topic, tag, date_yyyymmdd FROM _bronze_window GROUP BY 1,2,3,4,5 HAVING COUNT(*)>1))
