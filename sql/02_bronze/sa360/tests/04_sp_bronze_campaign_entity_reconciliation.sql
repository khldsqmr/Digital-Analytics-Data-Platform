...
PURPOSE:
  Reconcile Bronze Campaign Entity vs source (lookback window).
...
SET v_reason = IF(
  v_status='FAIL',
  CONCAT('Entity row count mismatch in last ', CAST(v_lookback_days AS STRING),
...
SET v_next = IF(
  v_status='FAIL',
  'Verify entity MERGE lookback window, dedup ordering, and late-arriving snapshots.',
  'No action required.'
);
...
-- =====================================================
-- TEST 2: Bidding Strategy Type Coverage (distinct count)
-- =====================================================
SET v_expected = (
...
SET v_next = IF(
  v_status='FAIL',
  'Check transformation/mapping for bidding_strategy_type in entity Bronze; validate source pull completeness.',
  'No action required.'
);
...
-- =====================================================
-- TEST 3 (optional): Advertising Channel Type Coverage
-- =====================================================
SET v_expected = (
  SELECT COUNT(DISTINCT CAST(s.advertising_channel_type AS STRING))
  FROM `prj-dbi-prd-1.ds_dbi_improvado_master.google_search_ads_360_beta_campaign_entity_custom_tmo` s
  WHERE SAFE.PARSE_DATE('%Y%m%d', CAST(s.date_yyyymmdd AS STRING)) >= v_window_start
);

SET v_actual = (
  SELECT COUNT(DISTINCT CAST(b.advertising_channel_type AS STRING))
  FROM `prj-dbi-prd-1.ds_dbi_digitalmedia_automation.sdi_bronze_sa360_campaign_entity` b
  WHERE b.date >= v_window_start
    AND b.date IS NOT NULL
);
...
SET v_reason = IF(
  v_status='FAIL',
  CONCAT('Distinct advertising_channel_type mismatch in last ', CAST(v_lookback_days AS STRING), ' days.'),
  'Advertising channel type coverage aligned.'
);

SET v_next = IF(
  v_status='FAIL',
  'Check mapping for advertising_channel_type; validate late snapshots / ingestion completeness.',
  'No action required.'
);
...
SELECT
  ...
  'sdi_bronze_sa360_campaign_entity',
  'reconciliation',
  CONCAT('Advertising Channel Type Coverage (', CAST(v_lookback_days AS STRING), '-day)'),
  'LOW',
  v_expected, v_actual, v_variance,
  v_status, v_emoji,
  v_reason, v_next,
