...
BEGIN
  ...
  USING DELTA
  COMMENT 'MFC Silver Spend (non-granular / LOB level) — refreshed via sdi_sp_mfc_silver_spend_weekly.'
  AS
  WITH
  ...
  actual_quarter_totals AS (
    ...
    FROM prdrzranalytics.lab42.sdi_tbl_mfc_bronze_spendActuals_weekly
    GROUP BY Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, LOB_Supported, Quarter
  ),
  actual_totals AS (
    SELECT
      Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, LOB_Supported,
  ...
  forecast_quarter_totals AS (
    SELECT
      ...
      MAX(FileLoad_Date) AS FileLoad_Date,
      MAX(Source_File_Date) AS Source_File_Date
    FROM prdrzranalytics.lab42.sdi_tbl_mfc_bronze_spendForecast_weekly
    GROUP BY Week_Beginning_Monday, Week_Ending_Sunday, QGP_Week, LOB_Supported, Quarter
  ),
  forecast_totals AS (
    SELECT
